        ...
        <el-tabs v-model="activeTab" type="border-card" @tab-click="handleTabClick">
          ...
          <el-tab-pane label="兼职管理" name="tabJzUser">
            ...
            >
              <el-table-column label="姓名" width="130px" prop="userName" sortable>
                <template slot-scope="{row}">
                  <span v-if="checkPermission(['selfkh_user_view']) === false || tree.checkedNode.id <= 0">{{ row.userName }}</span>
                  <router-link v-if="checkPermission(['selfkh_user_view']) === true && tree.checkedNode.id > 0" :to="'/user/jz-user-view/'+row.userId+'/'+row.userUuid" class="link-type">
          ...
          <el-tab-pane label="离职管理" name="tabLzUser">
            <div class="filter-container">
              ...
              <el-input v-model="lz.listQuery.rules.account.fv" placeholder="登录账号 精确匹配" style="width: 150px;" class="filter-item" />
