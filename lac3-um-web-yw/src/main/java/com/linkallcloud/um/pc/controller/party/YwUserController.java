...
public class YwUserController
    ...
    public @ResponseBody
    Result<Page<YwUser>> page4Role(@RequestBody WebPage webPage, Trace t, AppVisitor av)
    ...
    public @ResponseBody
    Result<Page<YwUser>> page4SysRole(@RequestBody WebPage webPage, Trace t, AppVisitor av)
            throws IllegalParameterException {
        Page<YwUser> page = webPage.toPage();
        if (!page.hasRule4Field("roleId") || !page.hasRule4Field("roleUuid")) {
            throw new IllegalParameterException(Exceptions.CODE_ERROR_PARAMETER, "roleId,roleUuid参数错误。");
    ...
    public @ResponseBody
    Result<Object> addRoles(@RequestParam(value = "userId", required = false) Long userId,
    ...
    public @ResponseBody
    Result<Object> removeRoles(@RequestParam(value = "userId", required = false) Long userId,
                               @RequestParam(value = "userUuid", required = false) String userUuid,
                               @RequestBody Map<String, Long> roleUuidIds, Trace t, ModelMap modelMap) {
    ...
    public @ResponseBody
    Result<Object> associate(@RequestParam(value = "id", required = false) Long id,
    ...
    public @ResponseBody
    Result<Object> disassociate(@RequestParam(value = "id", required = false) Long id,
                                @RequestParam(value = "uuid", required = false) String uuid, Trace t, ModelMap modelMap) {
        boolean ret = false;
        YwUser ywUser = ywUserManager.fetchByIdUuid(t, id, uuid);
