...
(function($) {
	...
	function setFieldErrorParentCss(field, prompText) {
		var parent = field.parents(".form-group");// getParentWithClassName(field, "form-group");
		if (!parent){
			return;
		}
		parent.addClass("has-error");
		var errorMessage = field.attr('errorMsg') ? field.attr('errorMsg') : prompText;
		if (0 < parent.find(".help-block").length)
			parent.find(".help-block").remove();
		if (0 == parent.find(".help-block").length)
			parent.append('<div class="col-xs-12 help-block"><small class="help-block">'+ errorMessage + '</small></div>');
	...
	function getParentWithClassName(field, className) {
		...
		if (parent && !parent.hasClass(className))
			parent = parent.parent();
			*/
		return field.parents("."+className);
	}
	...
	function setGroupFieldErrorParentCss(field, prompText) {
		...
		if (0 == parent.find('#' + field.attr("id") + '-error').length) {
			parent.append('<div class="errorInf" id="' + field.attr("id") + '-error">' + errorMessage + '</div>');
		}
	}
