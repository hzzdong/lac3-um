    ...
    <el-tabs v-model="activeTab" :tab-position="tabPosition" style="height: 100%;" @tab-click="handleTabClick">
      <el-tab-pane label="角色信息" name="tabRole">
        <div class="filter-container tool-bar">
          ...
          <el-button v-permission="['kh_sys_role_m']" type="success" icon="el-icon-edit" circle @click="toRoleUpdate()" />
          <el-button v-if="role.status === 0 && checkPermission(['kh_sys_role_m'])" type="warning" icon="el-icon-lock" title="禁用" circle @click="toChangeRoleStatus(1)" />
          <el-button v-if="role.status === 1 && checkPermission(['kh_sys_role_m'])" type="warning" icon="el-icon-unlock" title="启用" circle @click="toChangeRoleStatus(0)" />
          <el-button v-permission="['kh_sys_role_m']" type="danger" icon="el-icon-delete" circle @click="onRoleDelete()" />
        </div>
      ...
      <el-tab-pane label="角色分配" name="tabRoleUsers">
        ...
        <div class="filter-container">
          ...
          <el-button
            v-permission="['kh_sys_role_users']"
            class="filter-item"
            style="float: right;"
            type="primary"
            icon="el-icon-plus"
        ...
        >
          ...
          <el-table-column v-if="checkPermission(['kh_sys_role_users'])" label="操作" align="center" width="60" class-name="small-padding">
            <template slot-scope="{row}">
      ...
      <el-tab-pane label="应用授权" name="tabRoleApps">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <div class="filter-container">
              <el-button
                v-permission="['kh_sys_role_app_perm']"
                class="filter-item"
...
import { findRoleUsers, findUnRoleUsers } from '@/api/khuser'
import { findAppPage4SysKhRole, findAppPage4UnSysKhRole } from '@/api/application'
...
export default {
  ...
  methods: {
    ...
    loadMenuTree(roleId, roleUuid, appId, appUuid) {
      ...
      getPermedMenuTree4SysKhRole(req).then(response => {
