  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="角色名称" width="250px" prop="name" sortable>
        <template slot-scope="{row}">
          <span v-if="checkPermission(['kh_sys_role_m']) === false">{{ row.name }}</span>
          <router-link v-if="checkPermission(['kh_sys_role_m']) === true" :to="'/role/sys-kh-role-view/'+row.id+'/'+row.uuid" class="link-type">
            <span>{{ row.name }}</span>
          </router-link>
        </template>
      </el-table-column>
...
<script>
import { findCompanyRolePage, createSysKhRole } from '@/api/syskhrole'
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          createSysKhRole(tempData).then((response) => {
