    ...
    <el-tabs v-model="activeTab" :tab-position="tabPosition" style="height: 100%;" @tab-click="handleTabClick">
      <el-tab-pane label="部门基本信息" name="tabDepartment">
        <div class="filter-container tool-bar">
          ...
          <el-button v-permission="['yw_org_edit']" type="success" icon="el-icon-edit" circle @click="toDepUpdate()" />
          <el-button
            v-if="dep.status === 0 && checkPermission(['yw_org_edit']) === true"
            type="warning"
            icon="el-icon-lock"
            title="禁用"
            circle
          ...
          />
          <el-button
            v-if="dep.status === 1 && checkPermission(['yw_org_edit']) === true"
            type="warning"
            icon="el-icon-unlock"
            title="启用"
            circle
        ...
        <el-form ref="departmentViewForm" :model="dep" size="small" status-icon label-position="right" label-width="120px" style="width: 100%;">
