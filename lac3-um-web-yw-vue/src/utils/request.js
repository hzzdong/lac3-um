  ...
  config => {
    ...
    if (config.data) {
      if (config.data.dataType && config.data.dataType === 'Object') {
        const faceReq = { appCode: 'lac_app_um', data: config.data }
        config.data = faceReq
      } else {
        ...
        if (data.rules || data.orderby || (data.page && data.limit)) {
          ...
          for (const field in data.rules) { // ListFaceRequest
            ...
            if (rule && rule.fv && rule.fv !== undefined && rule.fv !== '') {
              req.query.cnds.push({
                field: field,
                data: rule.fv,
                type: rule.stype,
          ...
          const faceReq = Object.assign({ appCode: 'lac_app_um' }, config.data)
          config.data = faceReq
        }
      }
    } else {
      config.data = { appCode: 'lac_app_um' }
    }
    ...
    if (res.code !== '0') {
      ...
      if (res.code === 50008 || res.code === 50012 || res.code === 9021 || res.code === 'e.face.login') {
        ...
        store.dispatch('user/resetToken').then(() => {
          // location.reload()
          window.location = '/api4umyw/ssoauth'
        })
      }
