...
public class MenuActivity extends BaseTreeActivity<Menu, IMenuDao> implements IMenuActivity {
	...
	@Override
	public List<Tree> getMenus(Trace t, Sid appId, Boolean valid) {
		Application app = applicationDao.fetchByIdUuid(t, appId.getId(), appId.getUuid());
		if (app == null) {
			throw new BaseRuntimeException("80000001", "无法查询对应的应用，可能是您的参数有误。");
		}

		return assembleMenuTrees(t, app, valid);
	}
	...
	@Override
	public Tree getMenuTree(Trace t, Sid appId, Boolean valid) {
		Application app = applicationDao.fetchByIdUuid(t, appId.getId(), appId.getUuid());
		if (app == null) {
			throw new BaseRuntimeException("80000001", "无法查询对应的应用，可能是您的参数有误。");
		}
