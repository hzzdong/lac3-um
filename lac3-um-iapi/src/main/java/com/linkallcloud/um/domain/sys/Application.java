...
public class Application extends Domain {
	...
	/*
	 * 账号映射模式认证接口设置: 调用时机
	 * 根据以上设置，当用户首次访问此应用时，SSO会发起账户绑定操作。用户在绑定界面上输入账号信息后，SSO会以rest方式调用应用的账号认证接口 接口调用方式
	 * POST调用：账号认证接口地址?loginName=XXX&password=XXX&time=XXX$sign=XXX 参数说明
	 * （1）password=原文或者MD5(原文) （2）time=当前时间new Date().getTime()
	 * （3）sign=MD5/SHA1(loginName+password+time+签名密钥) 接口返回 接口返回JSON结果如下：
